...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", () => {
        ...
        navigator.clipboard.writeText(text).then(() => {
          window.showAppToast("ფრაზა დაკოპირდა ბუფერში");
        });
      });
    }
  };
  ...
  if (wordBridgeContainer && typeof WORD_BRIDGES !== "undefined") {
    ...
    WORD_BRIDGES.forEach(bridge => {
      ...
      if (copyBtn) {
        copyBtn.addEventListener("click", () => {
          navigator.clipboard.writeText(bridge.healing).then(() => {
            window.showAppToast("ფრაზა დაკოპირდა");
          });
        });
      }
  ...
  if (copyMemoBtn && typeof VISITOR_MEMO !== "undefined") {
    copyMemoBtn.addEventListener("click", () => {
      ...
      navigator.clipboard.writeText(text).then(() => {
        window.showAppToast("სტუმრის მემორანდუმი დაკოპირდა. შეგიძლიათ გაუგზავნოთ მესენჯერში.");
      });
    });
  }
